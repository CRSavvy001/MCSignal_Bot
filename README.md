...
- If the marketcap is at or below `MC_THRESHOLD`, the bot rebuilds the message as
  plain text — any hidden hyperlinks (e.g. the words "Web", "X", "TG" secretly
  linking somewhere) are rewritten as `Web (https://actual-url.com)` so the real
  destination is visible and still clickable — and sends that to `DEST_CHAT_ID`.
